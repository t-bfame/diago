...
func TestMetrics_Add(t *testing.T) {
	...
	codes := []uint32{500, 200, 302}
	errors := []string{"Internal server error", ""}

	got := NewMetricAggregator("testid", "instanceid", "jobid")

	for i := 1; i <= 10000; i++ {
		got.Add(&scheduler.Metrics{
...
// TODO: uncomment these later once we find a way to mock
// the NewLoadTestCollection function in aggregator.go
...
func BenchmarkMetrics(b *testing.B) {
	...
	for _, tc := range []struct {
		...
		m := NewMetricAggregator("testid", "instanceid", "jobid")
		m.Latencies = LatencyMetrics{estimator: tc.estimator}
		b.Run("Add/"+tc.name, func(b *testing.B) {
			for i := 0; i <= b.N; i++ {
				m.Add(&scheduler.Metrics{
					Code:      200,
